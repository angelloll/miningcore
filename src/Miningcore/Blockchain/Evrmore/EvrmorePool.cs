...
[CoinFamily(CoinFamily.Evrmore)]
public class EvrmorePool : PoolBase
{
    public EvrmorePool(IComponentContext ctx,
    ...
    private EvrmoreJobParams currentJobParams;
    private long currentJobId;
    private EvrmoreJobManager manager;
    private EvrmoreTemplate coin;
    ...
    protected virtual async Task OnSubscribeAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        var context = connection.ContextAs<EvrmoreWorkerContext>();
        ...
        var data = new object[]
        {
            new object[]
            {
                new object[] { EvrmoreStratumMethods.SetDifficulty, connection.ConnectionId },
                new object[] { EvrmoreStratumMethods.MiningNotify, connection.ConnectionId }
            }
        }
        ...
        // send intial update
        await connection.NotifyAsync(EvrmoreStratumMethods.SetDifficulty, new object[] { EvrmoreUtils.EncodeTarget(context.Difficulty) });
        await connection.NotifyAsync(EvrmoreStratumMethods.MiningNotify, minerJobParams);
    }
    ...
    {
        var context = connection.ContextAs<EvrmoreWorkerContext>();
        var job = new EvrmoreWorkerJob(NextJobId(), context.ExtraNonce1);
        ...
        var result = new object[]
        {
             ...
             headerHash,
             job.SeedHash,
             EvrmoreUtils.EncodeTarget(context.Difficulty),
             update,
             job.Height,
             job.Bits
        };
    ...
    protected virtual async Task OnNewJobAsync(object job)
    {
        ...
        currentJobParams = job as EvrmoreJobParams;
    ...
    {
        coin = pc.Template.As<EvrmoreTemplate>();
    ...
    {
        manager = ctx.Resolve<EvrmoreJobManager>(
            new TypedParameter(typeof(IExtraNonceProvider), new EvrmoreExtraNonceProvider(poolConfig.Id, clusterConfig.InstanceId)));
    ...
    {
        return new EvrmoreWorkerContext();
    }
        ...
        Timestamped<JsonRpcRequest> tsRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                case EvrmoreStratumMethods.Subscribe:
                    await OnSubscribeAsync(connection, tsRequest);
                    break;

                case EvrmoreStratumMethods.Authorize:
                    await OnAuthorizeAsync(connection, tsRequest, ct);
                    break;

                case EvrmoreStratumMethods.SubmitShare:
    ...
    protected override async Task OnVarDiffUpdateAsync(StratumConnection connection, double newDiff, CancellationToken ct)
    {
        ...
        {
            var minerJobParams = CreateWorkerJob(connection, currentJobParams.CleanJobs);
            await connection.NotifyAsync(EvrmoreStratumMethods.SetDifficulty, new object[] { EvrmoreUtils.EncodeTarget(connection.Context.Difficulty) });
            await connection.NotifyAsync(EvrmoreStratumMethods.MiningNotify, minerJobParams);
        }
    }
