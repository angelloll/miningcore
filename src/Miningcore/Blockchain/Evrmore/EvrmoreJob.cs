...
public class EvrmoreJobParams
{
    public ulong Height { get; init; }
    public bool CleanJobs { get; set; }
}

public class EvrmoreJob : BitcoinJob
{
    private EvrProgpowCache evrprogpowHasher;
    private new EvrmoreJobParams jobParams;
...
#pragma warning restore 618
    ...
    {
        var context = worker.ContextAs<EvrmoreWorkerContext>();
        ...
        if(!evrprogpowHasher.Compute(logger, (int) BlockTemplate.Height, headerHash.ToArray(), nonce, out var mixHashOut, out var resultBytes))
    ...
    public void Init(BlockTemplate blockTemplate, string jobId,
        ...
        IHashAlgorithm headerHasher, EvrProgpowCache evrprogpowHasher)
    {
        ...
        Contract.RequiresNonNull(evrprogpowHasher);
        Contract.Requires<ArgumentException>(!string.IsNullOrEmpty(jobId));

        this.coin = pc.Template.As<EvrmoreTemplate>();
        ...
        this.extraNoncePlaceHolderLength = EvrmoreConstants.ExtranoncePlaceHolderLength;
        ...
        this.evrprogpowHasher = evrprogpowHasher;
        ...
        this.jobParams = new EvrmoreJobParams
        {
    ...
    public void PrepareWorkerJob(EvrmoreWorkerJob workerJob, out string headerHash)
    {
        workerJob.Job = this;
        workerJob.Height = BlockTemplate.Height;
        workerJob.Bits = BlockTemplate.Bits;
        workerJob.SeedHash = evrprogpowHasher.SeedHash.ToHexString();
        headerHash = CreateHeaderHash(workerJob);
    }

    private string CreateHeaderHash(EvrmoreWorkerJob workerJob)
    {
